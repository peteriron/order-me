...
const ManageDrinks = () => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("customDrinks");
    if (stored) {
  ...
  const saveDrinks = (updatedDrinks: Drink[]) => {
    localStorage.setItem("customDrinks", JSON.stringify(updatedDrinks));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="p-6">
          <h2 className="text-xl font-semibold mb-4">Custom Drinks ({drinks.length})</h2>
          
          {drinks.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">
              No custom drinks yet. Add your first one above!
            </p>
          ) : (
